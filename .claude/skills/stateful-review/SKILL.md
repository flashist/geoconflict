...
Hand the novel, deduped findings to the `process-review` flow: classify **defect vs frontier-move**, verify each against the actual code, assign verdicts, flag any regression/oscillation **loudly and up front**, and gate every code change on explicit user approval. Reuse `process-review` — do not re-implement its evaluation here.
...
After the user decides, update `ai-agents/reviews/<task-id>.md` (Decision-log rows + any newly-accepted residuals with their structural *why* and *re-raise only if*), per `process-review` Step 5 and the reviews README. Create the ledger if it didn't exist.

---

## Hard rules

- Never change code without explicit approval — inherit `process-review`'s gate.
