...
export abstract class DefaultServerConfig implements ServerConfig {
  ...
  publicProtocol(): string {
    const runtime = this.runtimeConfig();
    if (runtime.publicProtocol && runtime.publicProtocol.trim().length > 0) {
      return runtime.publicProtocol.trim();
    }
    const envValue = process.env.PUBLIC_PROTOCOL;
    if (envValue && envValue.trim().length > 0) {
      return envValue.trim();
    }
  ...
  publicPort(): string {
    const runtime = this.runtimeConfig();
    if (runtime.publicPort && runtime.publicPort.trim().length > 0) {
      return runtime.publicPort.trim();
    }
    const envValue = process.env.PUBLIC_PORT;
    if (envValue && envValue.trim().length > 0) {
      return envValue.trim();
    }
    return this.publicProtocol() === "https" ? "443" : "80";
  }
  deploymentId(): string {
    const runtime = this.runtimeConfig();
    if (runtime.deploymentId && runtime.deploymentId.trim().length > 0) {
      return runtime.deploymentId.trim();
    }
    const envValue = process.env.DEPLOYMENT_ID;
    if (envValue && envValue.trim().length > 0) {
      return envValue.trim();
    }
  ...
  jwtIssuer(): string {
    const runtime = this.runtimeConfig();
    if (runtime.jwtIssuer && runtime.jwtIssuer.trim().length > 0) {
      return runtime.jwtIssuer.trim();
    }
    const envIssuer = process.env.JWT_ISSUER;
    if (envIssuer && envIssuer.trim().length > 0) {
      return envIssuer.trim();
    }
    const audience = this.jwtAudience();
  ...
  otelEnabled(): boolean {
    return Boolean(this.otelEndpoint());
  }
  otelEndpoint(): string {
