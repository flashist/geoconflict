...
export async function getServerConfigFromClient(): Promise<ServerConfig> {
  ...
  setRuntimeConfig({
    publicHost: config.publicHost ?? config.public_host,
    publicProtocol: config.publicProtocol ?? config.public_protocol,
  ...
  if (!gameEnv) {
    throw new Error("Missing game environment in /api/env response");
  }
