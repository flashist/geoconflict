...
**Board chosen honestly, and it is the owner's explicit instruction, not an inference:** the ruling was
*"File a brief, don't schedule."* Filing it on [`plan-sprint-4.md`](../../../sprints/plan-sprint-4.md)
would assert a sprint commitment the owner deliberately withheld. Same board reasoning as `0203`,
`0205` and `0207`.
...
This board is unranked by design, so its Priority column reads `—` and the rank lives here.
...
- **Still below a fix.** Unchanged reasoning: this only *measures*. `0210` closes a live farmable path
  that reaches a player-visible platform ranking; `0206` closed a silent XP loss. **Measuring never
  outranks fixing** — which is exactly why Medium and not higher, and why the non-gating rule above is
  not softened by this re-rank.
...
### ~~The three consumers~~ → **four** — who is waiting on this number
...
| [`0206`](../../done/0206-ffa-timer-expiry-award-to-top-client-player/brief.md) — ~~scheduled in Sprint 4~~ 📌 **✅ Done, closed 2026-09-03.** ⚠️ **Closed ≠ deployed — production state not verified this turn** | Its brief carries an explicit **unmeasured-frequency flag**: whether a clientless leader actually reaches the **80 %** FFA threshold in a real public lobby is recorded as **UNMEASURED, no production observation, no player report on file**. |
...
- **ADR-110** — `ai-agents/knowledge-base/decisions/adr-110-ai-player-may-be-declared-winner.md`.
  **Cited, not authored or edited here.** It carries a pre-committed revisit trigger; read it there.
