...
## Notes
- **152-ФЗ — identity is hashed AT THE PROFILE SERVER, not here (added by `s4-profile-hash-player-ids.md`, done).** `ProfileApiClient` sends the **raw** `yandexPlayerId` to `/internal/v1/profile/upsert` and `/internal/v1/credit` exactly as the wire contracts (`src/core/profile/CreditContract.ts`) define — the profile server hashes it on receipt (`src/profile-server/YandexIdHash.ts`) and persists only the irreversible hash. **Do NOT pre-hash on the game server**: the secret pepper lives only on the profile box (one tier, no divergence). The hashing is orthogonal to the SECURITY note below — a forged raw ID deterministically forges its hash, so signature verification is still required; the at-rest idempotency key is now `(game_id, yandex_player_id_hash)` but inherits the same forgeability exposure.
