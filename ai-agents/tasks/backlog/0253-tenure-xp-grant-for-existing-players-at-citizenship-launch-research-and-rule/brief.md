...
- **Depends on:** [`0273`](../0273-profile-identity-s4-client-login-session-and-bearer-token/brief.md) — profile identity **S4** (client login session + Bearer token), the last link of `0270` → `0271` → `0273` (updated 2026-09-15 when epic [`0266`](../0266-profile-identity-internal-player-id-platform-logins-login-endpoint/brief.md) was split into slices on owner rulings relayed by `fkit-lead`). 📌 **2026-09-23: `0273` is CODE-COMPLETE** (built + reviewed 2026-09-16, *"No code"* left). It moved to Sprint 5 for its **live check only**, so this dependency is **satisfied for the local rework**; only `0273`'s production proof is outstanding.
