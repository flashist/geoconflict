...
7. **Paid flags only from payment (security, 2026-06-13; updated — no migrate path):** confirm `is_paid_citizen = true` / `citizenship_purchased_at` can be produced ONLY by a verified purchase token via `/complete` or `/reconcile`. No other write path may set them: `upsertProfile` (first authenticated join) and `POST /internal/v1/credit` must force `is_paid_citizen = false` / `citizenship_purchased_at = null` regardless of any inbound value. *(The originally-cited `POST /v1/profile/migrate` forgery vector no longer exists — that endpoint was cancelled with T2/T7 on 2026-06-13.)*
...
- **Sole authority for paid citizenship (2026-06-13):** the verified Yandex payment flow (`/complete` and `/reconcile`, both gated on HMAC verification) is the ONLY path that may set `is_paid_citizen = true` / `citizenship_purchased_at`. These must never be set from any client-supplied body; every other write path (`upsertProfile` on first join, `POST /internal/v1/credit`) forces them off regardless of input. This is the deferred defense from the T1 PlayerProfile schema-contract review (2026-06-13): T1 stays a pure never-throw contract, so the paid-flags boundary is enforced entirely on the **writer side** (here + T5). *(The original "input side" half — the `POST /v1/profile/migrate` endpoint — was cancelled with T2/T7 on 2026-06-13; there is no migration input boundary, so writer-side enforcement is the sole remaining guard.)*
