...
## Depends on
**`0039-bots-hydrogen-bomb-sam-penetration-investigation` (the offset-H-bomb anti-SAM task).** Locked with Mark 2026-06-13: these two anti-SAM tactics are kept **separate**, and this one **builds on** the offset task. Reuse — do not re-derive — its shared outputs:
...
- Adjacent: `0036-bots-skip-sam-when-nukes-disabled` (also `FakeHumanExecution` SAM behaviour).
