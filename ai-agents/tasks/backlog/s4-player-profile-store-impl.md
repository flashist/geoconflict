...
**T4 internal order:** T4a–T4f ✅ done — the build/deploy pipeline (skeleton, client URL config, image, provisioning code, deploy mechanics, secret-scan gate) is merged. **Remaining:** **T4g** (argv/concurrency, after T4e/T4f), **T4h** (game-server `PROFILE_API_URL` env wiring — gates every runtime consumer, see constraint below), **T4i** (operator: actually bring the box up — run the merged T4d/T4e deploy + verify).
