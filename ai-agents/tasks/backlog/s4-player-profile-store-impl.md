...
> **T5 scope to revisit:** `POST /v1/profile/migrate` and its untrusted-body forcing (force paid flags off, recompute citizenship from carried XP) existed *for* T7's client→server upload. With no upload path, T5 likely reduces to fresh-profile-create + server-side crediting — no migrate endpoint, no migrate trust boundary. Confirm when T5 is picked up.
> Both slices can return later as one server-authoritative guest-XP cache (T2+T7 together).
