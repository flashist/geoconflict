...
**T4 internal order:** T4a–T4g ✅ done — the build/deploy + hardening pipeline (skeleton, client URL config, image, provisioning code, deploy mechanics, secret-scan gate, argv/concurrency hardening) is merged. **Remaining:** **T4h** (game-server `PROFILE_API_URL` env wiring — gates every runtime consumer, see constraint below), **T4i** (operator: actually bring the box up — run the merged T4d/T4e deploy + verify).
