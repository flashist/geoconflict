...
**Legal note (REOPENED 2026-06-28):** the 2026-06-26 conclusion (pseudonymize via an irreversible Yandex-ID hash) was **overturned** — further investigation found hashing does **not** remove the 152-ФЗ notification/consent obligation, only added complexity. The hashing task is **cancelled** (`s4-profile-hash-player-ids.md`, PR #127 reverted) and the findings doc (`ai-agents/knowledge-base/personal-data-152fz-findings.md`) is marked INVALIDATED. **152-ФЗ is unresolved again and gates profile-store production go-live** — a new task will determine the actual compliance approach (likely Roskomnadzor notification + consent).
...
**Parallel tracks (optional):** the original client track (T1 → T2) is **cancelled**; the backend track (T3 ✅ → T4 ✅ → T5 ✅ → T6) is the live path. T3, T4, and T5 are done (box live; profile DB + API merged, PR #126), so **T6 (match-end crediting) is now the next/unblocked slice**. **T6 is the production-verification gate** for the Citizenship Core UI task; T8 any time now (T4 done), before paid citizenship ships.
