...
- **`s4-starting-gold-public-modifier.md` (5M Starting Gold) is now fully independent of this task** (decoupled by Mark, 2026-06-20). It was previously framed as a *replacement* for infinite gold; that coupling is removed. The 5M starting-gold modifier ships on its own merits as an additional weird sub-option, and infinite gold stays (fixed via no-nukes here). The two tasks no longer depend on or sequence against each other.
