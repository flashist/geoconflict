...
> [`0211`](../../done/0211-credit-participation-xp-at-elimination-or-match-end/brief.md) —
...
> the same day; tracked on [`plan-sprint-4.md`](../../../sprints/done/plan-sprint-4.md), with its
...
>   [`0208`](../0208-measure-clientless-leader-at-win-condition-in-production/brief.md)
...
> [`plan-sprint-4.md`](../../../sprints/done/plan-sprint-4.md)/`-5`/`-6`) and of
...
| Is anything replacing it? | **Yes** — [`0211`](../../done/0211-credit-participation-xp-at-elimination-or-match-end/brief.md). ~~Unscheduled on the Backlog board.~~ ✅ **CORRECTED 2026-09-04 — SCHEDULED INTO SPRINT 4** (owner ruling, live in session), tracked on [`plan-sprint-4.md`](../../../sprints/done/plan-sprint-4.md); its Backlog-board row is kept as `➡️ Moved`. **Struck, not deleted — spent, not wrong.** Status **`🔲 Backlog`, NOT STARTED** (⚠️ scheduled is not started). Rank **`Medium–High`, the producer's, not an owner ruling.** ⛔ **Must NOT SHIP until [`0208`](../0208-measure-clientless-leader-at-win-condition-in-production/brief.md) is DEPLOYED AND COLLECTING DATA** — ⚠️ **not merely merged or built** — though ✅ **planning and building it in parallel is explicitly allowed.** ⚠️ **~~Neither task is `🚧 Blocked`.~~** 📌 **SWEPT 2026-09-04 by the producer — struck, not deleted; SPENT, NOT WRONG: accurate when written, false once `0208` was built.** ✅ **`0211` is still `🔲 Backlog` and still NOT blocked — only its SHIP is ordered.** ⛔ **`0208` IS now `🚧 Blocked` — built and reviewed 2026-09-04, UNCOMMITTED, UNDEPLOYED, NO DATA.** ⚠️ **Unrelated to the sequencing ruling; nothing gates `0208`.** *(Prose correction only — nothing else in this closed brief is touched.)* |
...
4. **The XP loss is still not fully closed.** If every clientful player is eliminated before the
   ...
   **100.0 %** of the map with every human eliminated and **the match still did not end.** Mechanism,
   ...
   [`0211`](../../done/0211-credit-participation-xp-at-elimination-or-match-end/brief.md)'s whole
...
5. **The `console.log` on the fallback award reaches no dashboard** — it runs in the client's Web
   ...
   [`0208`](../0208-measure-clientless-leader-at-win-condition-in-production/brief.md) is
...
| [`0208`](../0208-measure-clientless-leader-at-win-condition-in-production/brief.md) | Measure clientless-leader frequency at the win condition, in production | ~~Backlog — unscheduled~~ ✅ **CORRECTED 2026-09-04 — SCHEDULED INTO [Sprint 4](../../../sprints/done/plan-sprint-4.md)** (owner ruling, live in session); Backlog row kept as `➡️ Moved`. **Struck, not deleted — spent, not wrong.** ~~Status `🔲 Backlog`, **not started**;~~ 📌 **STATUS SWEPT 2026-09-04 by the producer — struck, not deleted; SPENT, NOT WRONG: it was true when written and was overtaken the SAME DAY.** ⛔ **`0208` WAS started: Part A and Part B are BUILT AND REVIEWED. Its status is now `🚧 Blocked` — UNCOMMITTED, UNDEPLOYED, NO DATA COLLECTED.** ⛔ **BUILT AND REVIEWED IS NOT DONE**, and 🔴 **`0211`'s gate — `0208` DEPLOYED AND COLLECTING DATA — is STILL UNSATISFIED.** *(Prose correction only — nothing else in this closed brief is touched.)* ~~rank `Medium`, the producer's.~~ 🔴 **RANK CORRECTED 2026-09-04 — the struck rank was WRONG WHEN WRITTEN (same session, by the producer), not spent-and-superseded. It was wrong on the VALUE and on WHOSE CALL IT WAS.** **Rank is `High`.** ⚠️ **The provenance is a SPLIT:** **THAT it was raised = an OWNER RULING** (2026-09-04, live in session); **THAT the value is `High` = the PRODUCER'S judgement** — the owner named no value. ⛔ **Do not collapse this into "the owner ranked it `High`" — they did not.** Authoritative: `0208`'s own *Priority*. ⛔ **Must be DEPLOYED AND COLLECTING DATA before [`0211`](../../done/0211-credit-participation-xp-at-elimination-or-match-end/brief.md) SHIPS.** |
...
- **Row appended, not inserted** on `backlog.md` (ADR-035) at filing — and again on
  [`plan-sprint-4.md`](../../../sprints/done/plan-sprint-4.md) at the 2026-09-03 promotion, for the same
...
- 🚩 **BOT TERRITORY CEILING — a structural finding from the 2026-09-03 play-test. It bears directly on
  [`0208`](../0208-measure-clientless-leader-at-win-condition-in-production/brief.md) and on
