...
## Priority
Medium. Depends on `0154-mini-mode-investigation` findings. If any maps are flagged as unsafe for compact mode, exclude them from the rotation (see Notes).
...
**This task also owns the shared modifier system** that the weird-setting task (`0156-weird-setting-modifier`) builds on. Implement the modifier system here first; the weird-setting task only adds entries to it.
...
const MATCH_MODIFIERS: MatchModifier[] = [
  ...
  // 'weird_setting' entry added by 0156-weird-setting-modifier
];
```
...
- Implement this task before `0156-weird-setting-modifier` — that task adds entries to `MATCH_MODIFIERS` and expects the system to already exist.
