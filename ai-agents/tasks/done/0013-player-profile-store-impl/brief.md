...
🔴 **CORRECTED 2026-09-04 — "the profile box is live" IS NOT CLAIMABLE.** ✅ **The rest of the struck sentence STANDS: all the code, scripts and slices really are done and merged — T4a–T4i, T5 (PR #126), T6, T8.** What is unknown is the **running state**, not the work.

**The reconciliation, from two owner statements the same day — both recorded, neither discarded:** first *"We don't have ANY profile-related VPS yet, we would need to have a full-scale setup for it (whatever is needed)"*, then, superseding it, *"We don't need to cancel any billings, the VPS and S3 I created will be reused"* — confirmed: *"Both exist — reuse them in place."* ⇒ ~~**A profile VPS and an S3 bucket PHYSICALLY EXIST and are reused in place**~~ — 🚨 **CORRECTED 2026-09-08 — THE S3 BUCKET IS NOT REUSED: a BRAND-NEW, CLEAN bucket is created (owner ruling, given live in session, superseding the 2026-09-04 reuse ruling AS TO THE BUCKET ONLY). ✅ The VPS half is UNCHANGED — the box is still reused in place.** ⇒ **A profile VPS PHYSICALLY EXISTS and is reused in place; whether the stack is provisioned and running on it is UNKNOWN AND UNVERIFIED.** ⚠️ **Hardware existence and provisioning state are two different facts, and only the first is known** — that gap is the owner's standing *"I am completely lost about what was done and what wasn't"*, not a contradiction.

The work to settle it is epic [`0213`](../../backlog/0213-profile-backend-clean-slate-rebuild/brief.md) — **wipe and rebuild onto the existing box and bucket**, phases P0–P7, all in Sprint 4 by owner ruling. ⛔ **This epic's `✅ Done` status is CORRECT and deliberate — do not "fix" it.** The work was done; its deployed state is a separate fact and belongs here, not in the status.
...
**T4 internal order:** T4a–T4i ✅ **done — T4 is complete.** The full build/deploy + hardening pipeline (skeleton, client URL config, image, provisioning code, deploy mechanics, secret-scan gate, argv/concurrency hardening, game-server `PROFILE_API_URL` env wiring) is merged, **and T4i stood up the box** (provision + DNS + deploy + 200/TLS verified). ~~`api.geoconflict.ru` is live.~~ 🔴 **CORRECTED 2026-09-04 — the host EXISTS, but its running state is UNVERIFIED and "is live" is not claimable.** T4i genuinely did stand a box up in June and genuinely did verify 200/TLS; **nobody has verified it since.** Read the past tense as history, not as a statement about production today. The box is being **reused in place** — see epic [`0213`](../../backlog/0213-profile-backend-clean-slate-rebuild/brief.md), whose P1 inspection fills in what is actually on it.
...
**Parallel tracks (optional):** the original client track (T1 → T2) is **cancelled**; the backend track (T3 ✅ → T4 ✅ → T5 ✅ → T6 ✅) is the live path. T3, T4, T5, and T6 are done (box live; profile DB + API merged, PR #126; match-end crediting shipped), so **T8 (backups) is now the next/unblocked slice**. **T6 was the production-verification gate** for the Citizenship Core UI task — now cleared; T8 any time now (T4 done), before paid citizenship ships.
