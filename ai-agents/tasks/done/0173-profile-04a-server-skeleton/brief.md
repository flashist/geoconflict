...
## Parent / Epic
`ai-agents/tasks/done/0013-player-profile-store-impl/brief.md` (Part D ops). One of **7 sub-tasks** split from `0172-profile-04-backend-infra` after the first attempt bounced — see postmortem `ai-agents/knowledge-base/profile-deploy-hardening-postmortem-2026-06-19.md`. Each sub-task merges independently and shrinks the review surface.
...
## Out of scope
- `/ready` readiness check and any DB-touching route → **T5** (now explicitly owned: `0185-profile-05-backend-db-api` Scope item 4 + Acceptance — DB-backed `SELECT 1`, distinct from this slice's liveness-only `/health`).
