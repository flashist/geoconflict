...
## Parent / Epic
`ai-agents/tasks/done/0013-player-profile-store-impl/brief.md` — child slice 5 of 8. Implements the **DB half of Part D** and the **profile-server half of Part E**.
...
> **If guest-XP is ever revived** (T2 + T7), the migration API (`POST /v1/profile/migrate`) and its untrusted-body hardening must be **re-added to this slice** — they were removed here, not merely deferred. See the cancelled tasks `0169-profile-02-guest-localstorage` / `0171-profile-07-guest-migration`, the epic cancellation note, and the T2 cancellation report (`ai-agents/knowledge-base/s4-profile-02-guest-localstorage-cancellation-2026-06-13.md`).
