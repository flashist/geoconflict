...
## Depends on
T3 (server-visible `yandexPlayerId`), T5 (the `/internal/v1/credit` endpoint), **T4h** (game-server `deploy.sh` must propagate `PROFILE_API_URL` into the container, or `ProfileApiClient`'s base URL resolves to `""` in prod — `0184-profile-04h-game-server-deploy-env`).
