...
- **Player Profile Store** must be live — the card reads XP from the **server** profile via `GET /v1/profile`. (Guest-first localStorage XP was cancelled 2026-06-13; XP is authenticated-only. ~~The profile store is now live, so this dependency is satisfied.~~ 🔴 **CORRECTED 2026-09-04 — this dependency is NOT demonstrated.** A profile VPS and S3 bucket **do exist and are being reused in place** (owner ruling, given live in session, superseding an earlier "we don't have any profile VPS" statement the same day — both recorded) — **but whether the stack is provisioned and serving is UNKNOWN AND UNVERIFIED.** ⚠️ **"Exists" is not "satisfied":** nobody has confirmed the card's server read would return anything. The inspection that settles it is epic [`0213`](../../backlog/0213-profile-backend-clean-slate-rebuild/brief.md)'s P1. ⛔ **This task's `✅ Done` status is CORRECT and deliberate — the UI was built and reviewed; its backing service is absent, which is a different fact. Do not "fix" the status.** ⚠️ **Consequence to state plainly: in production this card cannot show real XP today**, and it is additionally gated by `CITIZENSHIP_CARD_ENABLED` being `false`.)
