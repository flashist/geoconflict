...
## 2026-09-26 — ingest (sync)

- **Sync window:** `2df6bbf` → HEAD (`8f1f76b`). 4 commits (`28befdf`, `f67c2dd`, `e646362` "DEPLOY prod: bump version to 0.0.152", `8f1f76b`); tree clean at sync time.
- **Changed source files detected under `ai-agents/` (vault excluded): 80. Ingest-worthy after the filter: 41** (5 knowledge-base, 5 sprint boards, 31 done briefs). Skipped by the filter: 28 `tasks/backlog/*/brief.md` (incl. new **`0300`** — recorded from the board, not paged) and 10 in-folder `plan.md` / `worklog.md` / `review.md` working artifacts (mostly moved with their folders). `ai-agents/sprints/plan-sprint-4.md` shows as changed only because the board **moved** to `sprints/done/`; ingested once, at its new path.
- **Ingested:**
  - `ai-agents/tasks/done/0061-investigate-prod-telegram-feedback-delivery-failure/brief.md` → **created** [[wiki/tasks/feedback-telegram-delivery-failure]]
  - `ai-agents/tasks/done/0217-profile-p2-wire-game-server-to-profile-box/brief.md` → **created** [[wiki/tasks/profile-p2-wire-game-server]]
  - `ai-agents/tasks/done/0220-profile-p5-secret-persistence-and-value-parity/brief.md` → **created** [[wiki/tasks/profile-secret-persistence-value-parity]] (RUNBOOK-G sentence carried verbatim)
  - `ai-agents/tasks/done/0266-profile-identity-internal-player-id-platform-logins-login-endpoint/brief.md` → **created** [[wiki/tasks/profile-identity-epic]]
  - `ai-agents/tasks/done/0272-profile-identity-s3-game-server-resolve-and-credit-by-player-id/brief.md` → **created** [[wiki/tasks/profile-identity-s3-game-server-resolve-and-credit]]
  - `ai-agents/tasks/done/0273-profile-identity-s4-client-login-session-and-bearer-token/brief.md` → **created** [[wiki/tasks/profile-identity-s4-client-login-session]]
  - `ai-agents/tasks/done/0295-measure-the-game-prod-egress-ip-and-append-it-to-the-profile-internal-allowlist/brief.md` → **created** [[wiki/tasks/game-prod-egress-ip-allowlist]] (method only; no address anywhere)
  - `ai-agents/knowledge-base/weekend-deploy-slot-runbook.md` → **updated** [[wiki/systems/weekend-deploy-window]] — THE WINDOW RAN: W0–W14 results table, W16 Sprint 4 close, findings F-A…F-E (no fix decided for any), still-owed list; the 2026-09-25 keep-the-ping-URL reversal; the old *"expect `alerting: no`"* and *"W12 zero evidence"* claims annotated, not deleted.
  - `ai-agents/sprints/done/plan-sprint-4.md` → **updated** [[wiki/decisions/sprint-4]] — closed 2026-09-26 (agent-closed); **re-counted at `8f1f76b`: 106 rows — 85 Done · 18 Moved · 3 Cancelled; 0 open** (unchanged); Source path moved to `done/`. Only other board change: the `0014` row's test-account redaction.
  - `ai-agents/sprints/plan-sprint-5.md` → **updated** [[wiki/decisions/sprint-5]] — **re-counted: 28 rows — 7 Done · 15 Backlog · 6 Blocked; 21 open** (was 19 Backlog · 9 Blocked, 28 open); banner still `🔲 Backlog` — not started. ⚠️ Counted by me this run.
  - `ai-agents/sprints/backlog.md` → **updated** [[wiki/decisions/sprint-backlog]] — **88 rows, 67 open** (was 87 / 66); `0300` recorded from the board and its brief.
  - `ai-agents/knowledge-base/PROJECT.md` → **updated** [[wiki/systems/project-brief]] — the source's earned-path line corrected 2026-09-25 (1,000/10 → 100/1, ADR-111); dated note that the rescale and crediting are now in production, card still off.
  - `ai-agents/knowledge-base/decisions/adr-108-…`, `…/adr-114-…`, `ai-agents/knowledge-base/reports/2026-09-17-alert-rules-a1-a6-design.md`, `ai-agents/sprints/plan-index.md`, `ai-agents/sprints/plan-sprint-6.md` → **checked, no page change** — link paths only (`plan-sprint-4.md` → `done/`, `0061`/`0217` → `done/`).
  - `ai-agents/tasks/done/{0012,0014,0021,0062,0067,0182,0195,0206,0208,0211,0215,0222,0227,0241,0253,0270,0271,0274,0275,0276,0277,0282,0283,0284}/brief.md` → **checked, no page change** — link-path updates only (plus `0014`'s test-account redaction, already applied to the vault by the 2026-09-25 manual redaction).
- **Also updated (stale current-state claims, dated notes, history kept):** [[wiki/systems/player-profile-store]] (game server wired, credits + logins landing, F-A/F-B/F-C), [[wiki/features/feedback-button]] (delivery gotcha, `0300`), [[wiki/decisions/adr-112-free-xp-grants]] and [[wiki/tasks/tenure-xp-grant]] (🚨 claim-on-behalf risk OPEN from W3, 2026-09-26), [[wiki/decisions/adr-113-internal-player-id]], [[wiki/tasks/forward-profile-internal-token]] (`0296` A1–A4 ran), [[wiki/tasks/profile-match-end-crediting]], [[wiki/tasks/credit-participation-xp-elimination-or-match-end]], [[wiki/tasks/deploy-time-config-parity-guard]] and [[wiki/decisions/config-parity-failure-class]] (first real report-only prod run at W12), [[wiki/tasks/internal-path-case-variant-allowlist-bypass]] (11/11 → 403 re-run), [[wiki/systems/alert-delivery]], [[wiki/tasks/uptrace-alert-delivery-to-telegram]], [[wiki/decisions/incident-2026-08-22-public-lobbies-outage]] (`0061` closed). [[wiki/index]]: 7 new Tasks entries; 6 entries annotated (Sprint 4 no longer "THE ACTIVE SPRINT").
- **Targeted lint on 38 touched pages (+ `index.md`):** 0 broken wiki-links; 0 unresolved index links; every vault page has an index entry. **26 one-way links found and fixed** (all involving this run's new pages or new links). Moved-path sweep: every backtick `ai-agents/…` path in pages + index checked against the tree — only `sprints/plan-sprint-4.md` was broken (fixed on [[wiki/decisions/sprint-4]]'s Source line; one historical mention inside a struck note left as written); `sprints/.active-sprint` is an intentionally never-created file (ADR-108), not a defect. No vault page linked the moved `backlog/` task paths.
- 🔒 **Secret scan on the diff and new pages: clean** — no IP, URL, connection string, hex secret or test-account name; the egress address and image IDs deliberately omitted.
- ⛔ Wrote only inside `ai-agents/wiki-vault/`. Closed nothing, moved no task file, invoked no mover, edited no brief / sprint plan / knowledge-base file. Nothing committed or pushed.

## 2026-09-26 — ingest (sync)

- **Sync window:** `8f1f76b` → HEAD (`2177ea6`). 5 commits (`3386b90` the citizenship flip, `50eadad` "Sprint push", `917e571` / `5b3e6ec` "DEPLOY prod: bump version to 0.0.153 / 0.0.154", `2177ea6` "Sprint push"); tree clean at sync time.
- **Changed source files detected under `ai-agents/` (vault excluded): 69. Ingest-worthy after the filter: 19** (1 knowledge-base, 5 sprint boards, 13 done briefs). Skipped by the filter: 37 `tasks/backlog/*/brief.md` paths (incl. new **`0301`–`0318`**, recorded from the boards, not paged; and the pre-move `backlog/` paths of `0018`/`0065`, ingested at their `done/` paths) and 13 in-folder `plan.md` / `worklog.md` / `review.md` working artifacts. ⚠️ The four newly closed tasks' `worklog.md` files were **read as supporting evidence** for their pages, not ingested as sources.
- **Ingested:**
  - `ai-agents/tasks/done/0065-citizenship-paid-live-verification/brief.md` → **created** [[wiki/tasks/citizenship-go-live]]
  - `ai-agents/tasks/done/0018-citizenship-paid/brief.md` → **created** [[wiki/tasks/citizenship-paid]]
  - `ai-agents/tasks/done/0238-validate-citizenship-ui-kill-switch-in-a-real-build-launch-gate/brief.md` → **created** [[wiki/tasks/citizenship-kill-switch-launch-check]]
  - `ai-agents/tasks/done/0296-after-deploy-production-checks-profile-token-earned-citizenship-inbox/brief.md` → **created** [[wiki/tasks/after-deploy-production-checks]]
  - `ai-agents/sprints/plan-sprint-5.md` → **updated** [[wiki/decisions/sprint-5]] — renamed *Citizenship Launch*, started (`🔄 In progress — 2026-09-26`), 16 rows moved to Sprint 6, four closes. **Re-counted at `2177ea6`: 28 rows — 11 Done · 16 Moved · 1 Backlog; 1 open (`0297`).** ⚠️ Counted by me this run.
  - `ai-agents/sprints/plan-sprint-6.md` → **updated** [[wiki/decisions/sprint-6]] — renamed *Full F2P Loop & Social Features*; 16 rows in, 17 new briefs, `0248`/`0250` pulled in, four re-ranks. **Re-counted: 36 rows — 32 Backlog · 4 Blocked; all open** (was 5).
  - `ai-agents/sprints/backlog.md` → **updated** [[wiki/decisions/sprint-backlog]] — **93 rows, 70 open** (was 88 / 67); `0304`–`0306`, `0309`, `0310` added; `0248`, `0250` moved out.
  - `ai-agents/sprints/done/plan-sprint-4.md` → **updated** [[wiki/decisions/sprint-4]] — link paths + eight moved rows repointed to Sprint 6; **re-counted, unchanged: 106 rows, 0 open.**
  - `ai-agents/sprints/plan-index.md` → **updated** [[wiki/decisions/product-strategy]] — active sprint is Sprint 5; both renames; F2P items now Sprint 6.
  - `ai-agents/knowledge-base/weekend-deploy-slot-runbook.md` → **updated** [[wiki/systems/weekend-deploy-window]] — appended *Second game deploy*: `0.0.153` never served, `0.0.154` live, finding **F-F** (cause unknown, no owning task), W15 filter now `0.0.152` + `0.0.154`, `0268` clock.
  - `ai-agents/tasks/done/{0012,0014,0017,0061,0062,0195,0217,0253,0291}/brief.md` → **checked, no page change from the source** — link-path updates only (`backlog/` → `done/` for `0065`/`0238`/`0296`). Their pages that carried stale current-state claims were annotated below.
- **Also updated (stale pre-launch claims, dated banners, history kept):** [[wiki/tasks/hide-citizenship-card-flag]], [[wiki/tasks/citizenship-kill-switch-coverage]], [[wiki/tasks/citizenship-card-fail-closed-degraded-sdk]], [[wiki/tasks/yandex-catalog-registration]], [[wiki/tasks/citizenship-earned]], [[wiki/tasks/personal-inbox]], [[wiki/features/announcements]], [[wiki/systems/player-profile-store]], [[wiki/tasks/yandex-payments-implementation]], [[wiki/tasks/yandex-payments-secret-forwarding]], [[wiki/tasks/forward-profile-internal-token]], [[wiki/tasks/tenure-xp-grant]], [[wiki/decisions/adr-112-free-xp-grants]], [[wiki/systems/analytics]], [[wiki/tasks/analytics-p1-citizenship-funnel]], [[wiki/tasks/citizenship-name-change]], [[wiki/tasks/citizenship-xp-progress-ui]], [[wiki/systems/project-brief]]. [[wiki/index]]: 4 new Tasks entries; 24 entries annotated. [[wiki/decisions/product-strategy]]: one exact-duplicate Related line removed.
- **Verified in the tree this run:** `git status` clean at start; `CITIZENSHIP_CARD_ENABLED: true` in `src/client/flashist/FlashistFacade.ts`; `3386b90` touches only that file, its test and `0065`'s worklog.
- **Targeted lint on 34 pages (+ `index.md`):** 0 broken wiki-links; 0 unresolved index links; every vault page indexed. **33 one-way links found and fixed** (all involving this run's new pages or new banners). **Moved-path sweep:** no vault page or index line cited `backlog/` paths for `0065`, `0018`, `0238` or `0296` — nothing to repoint; the only unresolved backtick paths are the two known ones (`sprints/.active-sprint`, intentional per ADR-108; `sprints/plan-sprint-4.md` inside a struck historical note).
- 🔒 **Secret scan on the diff and new pages: clean** — no IP, URL, connection string, hex secret, account id prefix, game id or image tag.
- ⛔ Wrote only inside `ai-agents/wiki-vault/`. Closed nothing, moved no task file, invoked no mover, edited no brief / sprint plan / knowledge-base file. Nothing committed or pushed.
