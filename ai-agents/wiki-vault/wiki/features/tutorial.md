...
- [[systems/glossary]] — the lobby table recording that the tutorial is the one context where `disableNPCs` is forced `true` (`src/client/LocalServer.ts:115-121`), so Nations are never present here
...
- [[decisions/clientless-leader-win-policy]] — ⛔ task 0022's guard incidentally **fixed** a real tutorial bug: a bot winning a tutorial previously ran `reportPlacements()`, which ranks only Humans, so the single human player got **first-place POINTS on the real platform leaderboard for LOSING** (the `points`, not the `placement` rank — see the keep-them-apart table on [[decisions/clientless-leader-win-policy]]). Recorded so nobody "restores" the old behaviour. ~~✅ **RE-CHECK DISCHARGED 2026-09-03: `0206` shipped and did NOT reintroduce it** — its fallback award returns early on `gameType === GameType.Singleplayer`, so it is multiplayer-only and no tutorial can reach it.~~ 🔴 **CORRECTED 2026-09-04 (lint): `0206` was REVERTED and NEVER DEPLOYED — do not read "shipped" here.** ✅ **The re-check is still DISCHARGED, and now in a STRONGER sense: while `0206` existed in the repo its award was multiplayer-only (`gameType === GameType.Singleplayer` returned early); since the revert there is no award branch in `WinCheckExecution.ts` at all, so there is nothing left that could reintroduce the bug.** ⚠️ **The `0022` guard is a DIFFERENT thing and it STAYS — the revert did not touch it**, so the fix described above is intact. ~~📌 **`0206` was scheduled into Sprint 4 on 2026-09-03** (still `🔲 Backlog`, unstarted), so this hard re-check is now on a scheduled task's critical path.~~ 🚨 **A SEPARATE first-place-for-losing bug survives, and it is NOT this one:** in **non-tutorial Singleplayer**, losing to a bot still awards **10 real platform points**, and it is **farmable**. That is task `0210` (owner-ruled), unscheduled. ℹ️ The Team-mode twin [[tasks/teams-bot-team-win-stall]] does **not** carry it — a tutorial is created `gameMode: GameMode.FFA`, so `checkWinnerTeam()` can never run in one (✅ verified 2026-09-03)
- [[tasks/win-check-clientless-leader-guard]] — task 0022, which shipped that guard
- [[tasks/ffa-clientless-leader-fallback-award]] — task `0206`, 🔴 **REVERTED 2026-09-04, never deployed.** Its award returned early on `GameType.Singleplayer` and therefore could never have reached a tutorial; the hard re-check is **discharged**, and moot besides — the award no longer exists in the tree. Read its STOP box
- [[tasks/singleplayer-leaderboard-reporting-policy]] — task `0210`, the SAME first-place-for-losing shape surviving in **non-tutorial** Singleplayer, where it is farmable and still live
