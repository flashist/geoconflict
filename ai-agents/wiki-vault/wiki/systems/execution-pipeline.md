...
- [[decisions/clientless-leader-win-policy]] — why a suppressed `Win` update silently costs the whole match's XP. 🚨 **Still live and fixed nowhere as of 2026-09-04**; the planned fix is no longer an award but `0211`, which credits participation **independently of any winner**
...
- [[decisions/adr-110-ai-winner-allowed]] — which players a restored `Win` update **would** be allowed to name. 🔴 **The ADR is `accepted` and NOT superseded, but as of 2026-09-04 it rules on a predicate that exists in NO shipped code** — no `Win` update is restored on this path today
- [[tasks/ffa-clientless-leader-fallback-award]] — task `0206`, which restored the `Win` update on the FFA clientless-leader path by awarding a fallback winner — 🔴 **REVERTED 2026-09-04 on an owner ruling and NEVER DEPLOYED. The `Win` update is NOT restored; the `0022` guard's early `return` is what this path does today.** ⛔ Its `Status: done` records the WORK, not a live effect — read its STOP box. ⚠️ The revert deliberately kept one ADR-110 comment in `WinCheckExecution.ts`, so that file is **not** byte-identical to its pre-`0206` state
