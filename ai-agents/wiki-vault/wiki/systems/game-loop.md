...
- [[tasks/win-check-clientless-leader-guard]] — task 0022; before its guard, a clientless leader permanently deactivated the win check, leaving the match running with no way left to end it
- [[systems/client-game-teardown]] — 🚨 **a throw inside `GameRunner.executeNextTick()` reaches nothing.** `GameRunner.ts:171-183` builds an `ErrorUpdate`, but `Worker.worker.ts:20-28` drops it (`if (!("updates" in gu)) return;`) and the wire format has no worker→main error type, so the client's crash branch is **unreachable dead code**. Task `0232`, open. ⚠️ **The consequence — no modal, no teardown, an apparently frozen game — is REASONED FROM CODE and has never been observed**; whether the game freezes, recovers or diverges is unknown
