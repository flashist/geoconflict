...
- **Question 1 (upstream OpenFront API)** — **partly answered, owner, 2026-08-09:** it is **infrastructure to be self-hosted eventually** — not a dead leftover to rip out, and not a permanent dependency. ⚠️ **Whether anything actually reaches it at runtime is still unverified** and must not be asserted either way; task `0009` determines it.
  🚩 **READ Q1's SECOND CLAUSE AS THE CONDITIONAL IT IS — it has already produced one false dependency.** In the source document (`ai-agents/knowledge-base/architecture.md` §13 Q1) this question also says *"This determines whether R2 and the archive task are blocked on an external party"*. **That sentence was flattened into a flat "Blocks" and gave task `0009` a declared dependency on `0030` that does not exist.** ✅ **Corrected 2026-09-22:** an architect investigation, **accepted by the owner**, returned **`0009` does NOT block `0030`** — ⛔ **the determination was never run**, and 🔄 **`0030` REPLACES the archive POST with an S3 write, so it removes that leg from `0009`'s scope rather than waiting on it; the arrow points the other way.** **The discriminator:** citizen-gating reads `is_citizen` from the **profile server**, not from upstream flares / `PrivilegeChecker` — **had it keyed off the latter, the dependency would be real.** ⚠️ **Accepted, not proven — ~90 % on the technical verdict, ~70 % on intent**, because ⛔ code cannot establish what *"Blocks"* meant to its author on 2026-08-09. 🚨 **THE SEEDING SENTENCE IS STILL IN `architecture.md` BY DELIBERATE OWNER CHOICE** — they were offered the correction and **declined** — so **expect to meet it again and do not re-derive the dependency from it.** ⚠️ **This does NOT touch the `0008` / `PrivilegeRefresher` dependency on `0009` recorded under *Ranked risks* above — that one is real and unchanged.** See [[decisions/sprint-4]].
