...
- **Citizenship** — the supporter tier and the spine of the monetization layer. Two paths: *earned* (~~1,000 XP at 10 XP per qualifying match~~ 🔴 **RULED 2026-09-10: `1 XP` per qualifying match and the threshold ÷ EXACTLY 10 ⇒ `100 XP`; ⛔ *"about 10×"* is not the ruling, and time-to-citizenship is UNCHANGED at ~100 matches.** ⚠️ **NOT SHIPPED — the code today still reads `XP_PER_MATCH = 10` / `CITIZENSHIP_XP_THRESHOLD = 1000` in `src/core/profile/Citizenship.ts`; the rescale, including the player-facing `en.json`/`ru.json` copy, ships inside `0211` — see [[tasks/credit-participation-xp-elimination-or-match-end]]**) and *paid* (99 ₽). Any purchase grants citizenship. Benefits include no interstitial ads for paid citizens, the full emoji set, and further perks (name change, verified icon, private lobbies, spectating) planned behind it. See [[systems/player-profile-store]].
