...
- **`0014` is the upstream fact for `0195`.** Until Yandex issues the per-game secret key, the correct configured state for that variable is absent/empty and 503 is the correct behavior. 🔴 **OBSERVATION 2026-09-19, UNEXPLAINED AND FLAGGED, NOT RESOLVED: the box carries a NON-EMPTY value of length 32, and nobody has established where it came from.** ⚠️ **It is NOT known that `0014` issued a key** — a placeholder of the same length is indistinguishable from the outside. 🚩 **Someone should establish the value's provenance**; until then, treat the payments path as *unblocked-looking and unproven*, which is the worst of the three states to be in silently. Fixing the plumbing first means the key is not blocked on a deploy-script change when it arrives.
