...
- The config-drift sweep around the outage (§9 of the record) surfaced three production defects unrelated to the outage itself, each now a task: **`0062`** — `PROFILE_INTERNAL_TOKEN` is never forwarded by `deploy.sh`, so every profile call no-ops in prod (no profile row, no XP ever credited; blocks `0017` and `0018`); **`0063`** — prod `/api/env` advertised `http` on a raw IP (✅ **fixed and deployed 2026-08-29 in `362a2f9`**; ⚠️ the "silently breaking token login and profile fetch" framing filed here was later found **vacuous** — there is no auth service in this deployment, so no user ever held a token. The config was wrong and is now right, but do not repeat the user-impact claim — see [[tasks/prod-api-env-https-apex]]); **`0061`** — prod Telegram feedback delivery fails (📌 *closed 2026-09-26, agent-closed — delivery works after a retry-once fix; the cause was never confirmed in code; see [[tasks/feedback-telegram-delivery-failure]]*) (`TypeError: fetch failed`; the initial "needs `TELEGRAM_PROXY_URL`" diagnosis was disproven — the proxy is wired and forwarded, so it is an open investigation). Plus **`0060`** — container log retention now shared with nginx access logs nearly cost the investigation window. ✅ **`0060` shipped 2026-09-01** (agent-closed — not owner-verified) and **is not deployed**: retention flags at `update.sh:90-92` plus `access_log off;` on the 1-second-poll `/api/public_lobbies` block. 🔧 **Two corrections it produced, both against claims filed here.** The log configuration **is** in this repository — the "not in this repo, possibly a server-side change with no commit" premise was **refuted**; it shipped as an ordinary two-file commit. And ⚠️ **the pre-change retention figure this record circulates is UNVERIFIED** — it appears in no repo artifact and has never been confirmed with `docker inspect`, so do not repeat it as fact or use it as a baseline. See [[tasks/container-log-retention]].
...
- [[tasks/feedback-telegram-delivery-failure]] — task `0061`, the §9 feedback-delivery follow-up, closed 2026-09-26
