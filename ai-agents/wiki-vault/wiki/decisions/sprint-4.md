...
| Name Change (Citizens Only) | backlog | 0067 — first user-facing citizenship benefit; brief scoped 2026-08-24 (owner-ruled) as a request → moderation → apply loop. Inbox hooks stay seams per `0012`'s deferral |
| Citizen Verified Icon | backlog | 0068 — visible identity/status marker in lobbies and match UI; brief scoped 2026-08-24 (owner-ruled) as a server-sourced `isCitizen` flag plus an icon in the lobby and match player lists. Design TBD, placeholder glyph |
...
- **The 2026-08-22 production outage added an outage track to this sprint** (`0055` shipped → `0057` investigation → `0056` root-cause fix → `0192` routing fix → `0194` orphan fix, with `0193`'s poll guard alongside), plus three promoted config-drift findings (`0062`, `0063`, `0060`) from the same sweep; `0058`/`0059`/`0061` stayed unsprinted. **The track finished 2026-08-28, all six agent-closed and none owner-verified, none confirmed deployed.** Full chain and rulings: [[decisions/incident-2026-08-22-public-lobbies-outage]].
