...
- **`0195`** — `YANDEX_PAYMENTS_SECRET` is missing from `build-deploy-profile.sh`'s export block, so payments have been silently 503 ~~on the real box~~ (🔴 **2026-09-04: no real box exists** — owner-ruled; read it as the deployed configuration on the box that then stood) since `0019`. 🔴 **NO LONGER TRUE OF THE BOX TODAY — corrected 2026-09-19/20:** a non-empty `YANDEX_PAYMENTS_SECRET` (length 32, content never read) **is** present in the running container, the payments prefix answers **404, not 503**, on a missing sub-path, and ✅ **the provenance is answered — OWNER-ATTESTED (not repo-verified): the owner set the real Yandex per-game key themselves.** ⛔ **Presence and provenance only — correctness is unproven and NO real purchase has ever been exercised**; the `0195` code fix stands and the fail-closed code is unchanged. See [[decisions/config-parity-failure-class]].
