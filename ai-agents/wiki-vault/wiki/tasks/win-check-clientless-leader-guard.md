...
>
> 🚨 **The largest defect it uncovered is STILL LIVE IN PRODUCTION.** `0022` shipped the guard only.
> The XP that the guard's shape leaves unawarded is closed by `0206`, which is ~~**unscheduled and
> unstarted**~~ ~~**PROMOTED INTO SPRINT 4 on 2026-09-03** (owner ruling, live in session) but **still
> `🔲 Backlog` and still unstarted**~~ ✅ **BUILT, REVIEWED AND CLOSED 2026-09-03** as
> `✅ Done (agent-closed — not owner-verified)`. **Struck, not deleted.**
>
> 🔴 **"Still live in production" REMAINS TRUE.** `0206` shipped **into the repo only** — no deploy,
> no production observation, no owner play-test. And even in the repo the XP loss is **not fully
> closed**: where **every clientful player is eliminated before the threshold** the award finds nobody
> and the match's XP is still lost, knowingly. See [[decisions/clientless-leader-win-policy]].
...
| Change | Where |
|---|---|
| Risk 1 — clientless-leader guard, placed **before** `setWinner` and **before** `active = false` | `src/core/execution/WinCheckExecution.ts:65-73` (`setWinner` `:74`, `this.active = false` `:76`) |
...
- **R1 — `reportPlacements()` no longer fires for a clientless-leader FFA match.** ⚠️ **Not
  ...
  **worse** when nobody ever does. Owner-ruled an **accepted residual of the guard-only shape, not a
  ...
  ✅ **`0206` did not reintroduce this** — its fallback award returns early on
  `gameType === GameType.Singleplayer`, so it is multiplayer-only (verified against the tree
  2026-09-03).
...
- `0206` — the FFA timer/threshold award, which is what actually closes the XP loss above.
  📌 **PROMOTED INTO SPRINT 4 on 2026-09-03** on an owner ruling; its Backlog-board row is kept as
  `➡️ Moved`. ~~⚠️ **Scheduled is not started** — `🔲 Backlog` on that board, deliberately.~~
  ✅ **PLANNED, BUILT, REVIEWED AND CLOSED THE SAME DAY** — `✅ Done (agent-closed — not
  owner-verified)`. Brief now at
  `ai-agents/tasks/done/0206-ffa-timer-expiry-award-to-top-client-player/brief.md`.
  Codex adversarial pass: **"No findings."**; three **low** Claude findings, all dispositioned.
  `npm test` → 109 suites / 1133 tests green on first run. 🔴 **Nine residuals survive the close** —
  chiefly that **nothing has been run live**, the XP loss is **still open** where every clientful
  player is eliminated first, and **public FFA now ends at 80 %**, possibly crowning a player holding
  very little territory. It also spawned four briefs: `0207`, `0208`, `0209`, `0210`.
  ⚠️ **`0206` has no wiki task page yet** — creating it is an ingest, not this lint.
...
- [[decisions/sprint-backlog]] — where `0205` and `0207` are filed (`0206`'s row now reads `➡️ Moved`)
...
- [[tasks/ffa-clientless-leader-fallback-award]] — task `0206`, the award that closes the XP residual this task shipped by design
