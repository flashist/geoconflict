...
- [[decisions/adr-111-xp-economy-rescale]] — the `1 XP` / `100 XP` rescale, which will change the threshold this card displays. ⚠️ **This page is deliberately NOT rewritten to `100`: it records what `0191` BUILT, and that code still reads `CITIZENSHIP_XP_THRESHOLD = 1000`** (verified 2026-09-11 — nothing has shipped). The rescale also mandates the **player-facing copy in both `en.json` and `ru.json`** move in the same change (`0211` verification step `4d`)
