...
- Kept `<citizenship-card>` behind the `citizenship_ui` experiment flag and rendered its guest, logged-in progress, and citizen states from the shared `CITIZENSHIP_XP_THRESHOLD` constant — **`1000` when `0191` built it.** ✅ **The constant is `100` since 2026-09-12** ([[decisions/adr-111-xp-economy-rescale]], shipped by `0211`), so **the card now displays `100` with NOT ONE LINE OF THIS TASK'S CODE CHANGED** — reading from the shared constant is exactly why. ⛔ **The behaviour moved underneath this task; the build did not.**
