...
- [[decisions/licensing-compliance]] — AGPL, CC BY-SA, source-access, and OpenFront asset/trademark constraints for GeoConflict; the 0025 audit (2026-08-23) found one violation (proprietary music) — remediation 0066 **deployed 2026-08-29 in `362a2f9`**, so the paid-IAP gate is satisfied on the deploy fact, but its three live checks were **not run**

### Bug Fixes & Investigations
- [[decisions/windoworigin-url-join-defect]] — 🔧 **Fix DEPLOYED 2026-08-29 (`362a2f9`); task still NOT Done because its production proof is UNREACHABLE** — the private-lobby buttons are `display: none` on the Yandex template (task `0198`): `windowOrigin` is `origin + pathname`, so concatenating a worker route onto it misses the route on the `/yandex-games_iframe.html` production entry point — a PUT/POST 404s, a GET gets the SPA fallback's 200 — and private-lobby start silently fails on Yandex Games. Review closed 2026-08-28 at `🚧 Blocked — awaiting deploy proof`, **task NOT Done**, six owner-confirmed residuals. Carries the durable rule, the non-root testing requirement, and the corrected Express-not-nginx substitution mechanism
...
- [[decisions/config-parity-failure-class]] — the recurring class *a variable that never reaches production*: `0062`, `0063` and `0195`, the first outside `deploy.sh`; `0064` is the guard and must land after all three. **`0063` is fixed and deployed; `0062`'s fix shipped with the token deliberately BLANK**, so a presence check would pass on a variable doing nothing
...
- [[tasks/licensing-remediation]] — Sprint 4 task 0066 licensing remediation from the 0025 audit: proprietary-music purge, original placeholder favicon, JWT fallback retarget; agent-closed 2026-08-24, **DEPLOYED in `362a2f9`** — clears 0065's flip-ON gate on the deploy fact, live checks unrun
