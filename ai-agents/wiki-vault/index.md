...
- [[decisions/config-parity-failure-class]] — the recurring class *a variable that never reaches production*: `0062`, `0063` and `0195`, the first outside `deploy.sh`; `0064` is the guard and must land after all three. **`0063` is fixed, deployed and fully evidenced (2026-08-30); `0062`'s fix shipped with the token deliberately BLANK**, so a presence check would pass on a variable doing nothing
