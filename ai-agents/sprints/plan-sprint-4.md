...
| 🔄 In Progress | Player Profile Store — Implementation *(8-slice epic; T1 ✅ done, T2+T7 ⛔ cancelled 2026-06-13 — guest-first story dropped, now authenticated-only; T4 ✅ done; T3 next)* | `s4-player-profile-store-impl.md` |
...
- ✅ **T4 — Backend infra** (`s4-profile-04-backend-infra.md`) — dedicated reg.ru profile VPS deploy pipeline (`setup-profile.sh` / `build-deploy-profile.sh`), Postgres + API `/health` skeleton, and `PROFILE_API_URL` via `/api/env`; done & moved to `done/` (PR #112). Tooling-only — live box provisioning + DNS run separately.
- ⬜ **Next: T3 (Yandex identity)** — does not depend on T1; leads into T5/T6.
