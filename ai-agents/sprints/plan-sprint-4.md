...
| 🔄 In Progress | Player Profile Store — Implementation *(epic; T1 ✅, T3 ✅; T2+T7 ⛔ cancelled 2026-06-13 — guest-first dropped, now authenticated-only; T4 ✅ complete + T5 ✅ done 2026-06-24 (box live at api.geoconflict.ru; profile DB+API PR #126); next T6 → T8)* | `s4-player-profile-store-impl.md` |
...
| ✅ Done | Personal-Data Compliance (152-ФЗ) — Investigation *(decision 2026-06-26: pseudonymize via irreversible ID hash → findings + `s4-profile-hash-player-ids.md`)* | `s4-personal-data-compliance-investigation.md` |
| ✅ Done | Profile Store: Pseudonymize Player Identity — store an irreversible hash, not the raw Yandex ID *(code done & reviewed, PR #127; **remaining go-live gate: on-box raw-ID grep — confirm no raw ID at rest before T6 credits real players**)* | `s4-profile-hash-player-ids.md` |
...
**Outcome (decision 2026-06-26):** pseudonymize identity — store an **irreversible, unique hash** of the Yandex ID, never the raw ID. This avoids the Roskomnadzor notification + consent obligation for the identity data (residency already satisfied). The gate on profile-store prod go-live shifted from "file Roskomnadzor notification" to **implementing the hashing** → `s4-profile-hash-player-ids.md` (**done, PR #127**); the remaining go-live gate is now the **on-box raw-ID verification** (confirm no raw Yandex ID at rest). **Open item:** display-name (and other directly-identifying fields) handling is NOT resolved by ID-hashing — must be confirmed before prod (see findings). The historical investigation framing follows for context.
