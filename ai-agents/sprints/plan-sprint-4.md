...
| ⬜ Backlog | Profile Store: Pseudonymize Player Identity — store an irreversible hash, not the raw Yandex ID *(152-ФЗ decision; **gates profile-store prod go-live**; land before T6 credits real players)* | `s4-profile-hash-player-ids.md` |
...
**Outcome (decision 2026-06-26):** pseudonymize identity — store an **irreversible, unique hash** of the Yandex ID, never the raw ID. This avoids the Roskomnadzor notification + consent obligation for the identity data (residency already satisfied). The gate on profile-store prod go-live shifts from "file Roskomnadzor notification" to **implementing the hashing** → `s4-profile-hash-player-ids.md`. **Open item:** display-name (and other directly-identifying fields) handling is NOT resolved by ID-hashing — must be confirmed before prod (see findings). The historical investigation framing follows for context.
