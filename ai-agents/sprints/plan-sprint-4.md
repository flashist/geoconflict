...
| 🔄 In Progress | Player Profile Store — Implementation *(epic; T1 ✅, T3 ✅; T2+T7 ⛔ cancelled 2026-06-13 — guest-first dropped, now authenticated-only; T4 re-split into 9 sub-slices 2026-06-19 — T4a–T4f ✅ merged, T4g/T4h/T4i remain (T4i = operator VPS bring-up); then T5 → T6 → T8)* | `s4-player-profile-store-impl.md` |
...
- 🔄 **T4 — Backend Infra** — the monolithic T4 (`s4-profile-04-backend-infra.md`, PR #112) was **reverted** and re-decomposed into 9 ops sub-slices on 2026-06-19. **T4a–T4f are done & merged; T4g/T4h/T4i remain.** The parent `s4-profile-04-backend-infra.md` is retained as the T4 overview:
  ...
  - ⬜ **T4h — Game-server deploy env** (`s4-profile-04h-game-server-deploy-env.md`) — game-server `deploy.sh` must propagate `PROFILE_API_URL` into the container, else it resolves to `""` in prod. **Gates T6 + the Citizenship UI at runtime.**
  - ⬜ **T4i — Operator bring-up runbook** (`s4-profile-04i-server-bring-up-runbook.md`) — **operator action (Mark):** provision the reg.ru box, point `api.geoconflict.ru` DNS, run the merged deploy, verify 200/TLS. **Blocks T5 going live.**
