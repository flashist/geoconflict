...
| ⚠️ Reopened | Personal-Data Compliance (152-ФЗ) — Investigation *(⚠️ conclusion OVERTURNED 2026-06-28: hashing does NOT remove the 152-ФЗ obligation — hashing task cancelled. 152-ФЗ is UNRESOLVED again; a new task will handle it.)* | `s4-personal-data-compliance-investigation.md` |
| ⛔ Cancelled | Profile Store: Pseudonymize Player Identity — store an irreversible hash, not the raw Yandex ID *(⛔ Cancelled 2026-06-28 — further investigation found hashing does **not** remove the 152-ФЗ obligation; it only adds support/dev complexity for no benefit. PR #127 reverted. 152-ФЗ reopened → new task to follow.)* | `s4-profile-hash-player-ids.md` |
...
**Outcome — SUPERSEDED 2026-06-28.** The 2026-06-26 decision (pseudonymize via an irreversible Yandex-ID hash) was **overturned on further investigation: hashing does NOT remove the 152-ФЗ notification/consent obligation** — it only added support/dev complexity for no legal benefit. The hashing task (`s4-profile-hash-player-ids.md`) is **cancelled** and PR #127 was reverted. **152-ФЗ is therefore UNRESOLVED and reopened** — a new task will determine the real compliance path (likely Roskomnadzor notification + consent, or another approach). Profile-store production go-live is **gated on that new task**, not on hashing. The findings doc (`personal-data-152fz-findings.md`) is retained but marked **INVALIDATED**. The historical investigation framing follows for context.
