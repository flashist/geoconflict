...
| 🔄 In Progress | Player Profile Store — Implementation *(8-slice epic; T1 ✅ done, T2+T7 ⛔ cancelled 2026-06-13 — guest-first story dropped, now authenticated-only; T3/T4 next)* | `s4-player-profile-store-impl.md` |
...
**Status: 🔄 In Progress.** Split into 8 child slices (T1–T8) — see the slice table in `s4-player-profile-store-impl.md`. With T2 and T7 cancelled, the live path is the backend track (T3/T4 → T5 → T6); T6 remains the production-verification gate.
...
- ⬜ **Next: T3 (Yandex identity) and T4 (backend infra)** — neither depends on T1, so both can start now/in parallel; they lead into T5/T6.
- ⛔ **T7 — Guest→authenticated migration — cancelled 2026-06-13** (Mark), moved to `cancelled/`. With both T2 and T7 gone, the guest-first story (pre-login XP accumulation + migrate-on-login) is dropped: **profile XP is now authenticated-only** — only logged-in Yandex users accrue XP toward citizenship, server-side via T5/T6. Guests still get the locked citizenship card + login prompt (Part G, via T3), just no pre-login XP to carry over. **Watch T5 scope:** the `POST /v1/profile/migrate` endpoint and its untrusted-body hardening existed *for* T7 — with no client→server profile upload anymore, T5 likely reduces to fresh-profile-create + server-side crediting (no migrate endpoint / no migrate trust boundary). The guest-XP feature (both slices) can return later as one thin server-authoritative cache; deferred verification: epic #1 (guest XP increments), #2/#3 (migration), local half of #5.
