...
| 🔄 In Progress | Player Profile Store — Implementation *(epic; T1 ✅, T3 ✅; T2+T7 ⛔ cancelled 2026-06-13 — guest-first dropped, now authenticated-only; T4 re-split into 9 sub-slices 2026-06-19 — T4a–T4h ✅ merged (all engineering done), only T4i remains (operator VPS bring-up); then T5 → T6 → T8)* | `s4-player-profile-store-impl.md` |
...
**Status: 🔄 In Progress.** Originally 8 child slices (T1–T8); on 2026-06-19 the monolithic **T4 was reverted (PR #112) and re-decomposed into 9 ops sub-slices (T4a–T4i)** — see the slice tables in `s4-player-profile-store-impl.md` and `s4-profile-04-backend-infra.md`. **T4a–T4h are done & merged (as of 2026-06-24) — all T4 engineering complete; only T4i (operator bring-up) remains.** With T2 and T7 cancelled, the live path is the backend track (T3 ✅ → T4 → T5 → T6 → T8); T6 remains the production-verification gate.
...
- 🔄 **T4 — Backend Infra** — the monolithic T4 (`s4-profile-04-backend-infra.md`, PR #112) was **reverted** and re-decomposed into 9 ops sub-slices on 2026-06-19. **T4a–T4h are done & merged; only T4i (operator bring-up) remains.** The parent `s4-profile-04-backend-infra.md` is retained as the T4 overview:
