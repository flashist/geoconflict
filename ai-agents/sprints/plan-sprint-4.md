...
| 🔲 Backlog | — | Investigation: Win Condition Regression — Fill Bots Winning in Teams / HumansVsNations *(**row added 2026-09-02 on an owner ruling given live in session.** The brief already read `## Sprint: Sprint 4` and `Priority: High` since filing, but the task appeared in **no sprint file at all** — it was board-invisible, exactly the class `0196` swept for dependencies. This row makes it visible; it changes nothing about the brief, whose Sprint and Priority fields are untouched. Potential silent regression from `0140-solo-win-condition-fix` (PR #77) in `WinCheckExecution.ts` / `GameImpl.ts`: fill bots can win a Teams or HumansVsNations match when land is scarce, producing a result the player never asked for. **Investigation-first** — no implementation brief until findings are reviewed. **Row appended, not inserted** — this board is unranked (every Priority cell reads `—`), so row order encodes no rank, and appending avoids a mid-board insertion above the closed rows, which ADR-035 bars. ⚠️ **The brief's `High` priority therefore does NOT appear in this table's Priority column** — that column is `—` for every row on this unranked board; the rank lives in the brief)* | [`0022-win-check-multiplayer-regression-investigation`](../tasks/backlog/0022-win-check-multiplayer-regression-investigation/brief.md) |
