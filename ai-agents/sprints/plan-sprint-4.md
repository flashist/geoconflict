...
| 🔄 In Progress | Player Profile Store — Implementation *(epic; T1 ✅, T3 ✅; T2+T7 ⛔ cancelled 2026-06-13 — guest-first dropped, now authenticated-only; T4 reverted & re-split into 7 sub-slices T4a–T4g 2026-06-19, T4a ✅ merged, T4b/T4c ready next; then T5 → T6 → T8)* | `s4-player-profile-store-impl.md` |
...
**Status: 🔄 In Progress.** Originally 8 child slices (T1–T8); on 2026-06-19 the monolithic **T4 was reverted (PR #112) and re-decomposed into 7 ops sub-slices (T4a–T4g)** — see the slice tables in `s4-player-profile-store-impl.md` and `s4-profile-04-backend-infra.md`. With T2 and T7 cancelled, the live path is the backend track (T3 ✅ → T4 → T5 → T6 → T8); T6 remains the production-verification gate.
...
- 🔄 **T4 — Backend Infra** — the monolithic T4 (`s4-profile-04-backend-infra.md`, PR #112) was **reverted** and re-decomposed into 7 independently shippable ops sub-slices on 2026-06-19. The parent `s4-profile-04-backend-infra.md` is retained as the T4 overview; the sub-slices carry the executable scope:
  - ✅ **T4a — Server skeleton** (`s4-profile-04a-server-skeleton.md`) — Express `/health` skeleton in `src/profile-server/`, merged (PR #115). *Blocks T4c.*
  - ⬜ **T4b — Client API-URL config** (`s4-profile-04b-client-api-url-config.md`) — `PROFILE_API_URL` via `/api/env`. **No deps — ready now.**
  - ⬜ **T4c — Dockerfile** (`s4-profile-04c-dockerfile.md`) — image runs the T4a skeleton. **Unblocked by T4a — ready now.** *Blocks T4e, T4f.*
  - ⬜ **T4d — VPS provisioning + DNS** (`s4-profile-04d-vps-provisioning.md`) — **needs Mark to provision the reg.ru box + point the `api.geoconflict.ru` A-record.** *Blocks T4e.*
  - ⬜ **T4e — Deploy mechanics** (`s4-profile-04e-deploy-mechanics.md`) — `setup-profile.sh` + `build-deploy-profile.sh` + compose. *Depends T4c + T4d; blocks T4f, T4g.*
  - ⬜ **T4f — Image secret scan** (`s4-profile-04f-image-secret-scan.md`) — build-context leak gate. *Depends T4e; blocks T4g.*
  - ⬜ **T4g — argv/concurrency hardening** (`s4-profile-04g-argv-concurrency-hardening.md`) — *Depends T4e + T4f.*
