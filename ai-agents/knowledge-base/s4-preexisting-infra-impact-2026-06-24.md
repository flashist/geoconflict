...
| flags (§3.4) | Yes (one live bug) | **keep-adapt** | Subsystem is inert (disabled server-side, correct), but the `/flags` → `/flags_source` 404 breaks the live language selector AND the s4 citizenship-card flag image — fix the asset path now. |
...
### 3.4 Flags — keep-adapt

**Mechanism.** The flag feature is inert and *correctly* disabled server-side (`Privilege.ts:53-54` comments out `cosmetics.flag = result.data` — keep this, it's the right security posture). But commit `895368d` renamed `resources/flags/` → `resources/flags_source/` while every call site still requests `/flags/*.svg`. CopyPlugin preserves dir names (`webpack.config.js:347-360`), so `static/` has only `flags_source` → **every `/flags/*.svg` 404s build-wide.**

**Reasoning.** keep-adapt (overriding the first-pass keep-asis, per challenger). This is not s4-confined: the 404 breaks the **live, always-rendered language selector** (`LangSelector.ts:324`, `LanguageModal.ts:111`) in production at geoconflict.ru *today*, independent of s4. And the s4 `CitizenshipCard` flag image (`CitizenshipCard.ts:207,257-262`) shows a broken image for any player who used the live country picker (`FlagInputModal.ts:88` writes `localStorage["flag"]`) — the emoji fallback only fires on *empty* localStorage, and the card has no `onerror`. So it's the **common path**, not a corner case. Fix = asset-path correction (rename references back to `/flags`, or add a CopyPlugin `flags_source → /flags` mapping), NOT a flags rebuild. Do **NOT** remove `FLAG_STORAGE_KEY` / `FlagSchema` — the already-built s4 card imports them.

**Evidence.** `Privilege.ts:53-54`; `webpack.config.js:347-360`; `CitizenshipCard.ts:4,207,257-262`; `FlagInput.ts:104,110`; `LangSelector.ts:324`; `LanguageModal.ts:111`; `FlagInputModal.ts:88`; `index.html:182` / `yandex-games_iframe.html:288`.

**s4 tasks impacted.** s4-citizenship-xp-progress-ui, -citizenship-paid, -profile-05 (inert table), -player-profile-store-impl.
...
- **`/flags` → `/flags_source` 404 (build-wide).** Breaks the live language selector now AND the s4 citizenship-card flag image. **Fix the asset path independent of s4** (don't let it sit behind backlog tasks). It is the common path for any player who used the country picker, not an edge case.
