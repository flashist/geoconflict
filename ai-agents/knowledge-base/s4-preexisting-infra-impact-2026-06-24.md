...
3. **Backup-task conflict — RESOLVED 2026-06-29.** The two duplicate briefs (`s4-profile-08-backups`, profile box; `s4-postgres-backup-routine`, originally game VPS) were consolidated into a single canonical T8 brief, `s4-postgres-backup-routine.md`, retargeted to the dedicated profile box (`api.geoconflict.ru`) and the profile deploy path (`build-deploy-profile.sh` / `setup-profile.sh`); `s4-profile-08-backups.md` was removed. No further reconciliation needed.
