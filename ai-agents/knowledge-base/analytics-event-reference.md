...
| `uiElementIds.citizenshipLoginToEarn`  | `UI:Tap:CitizenshipLoginToEarn`  | Guest player taps the "Войти в Яндекс" login CTA on the citizenship card (start screen). Note: supersedes the `UI:Tap:CitizenLoginCta` string mentioned in `s4-citizenship-xp-progress-ui.md` — the citizenship funnel spec (`0021-analytics-p1-citizenship-funnel`) is authoritative |
| `uiElementIds.purchaseCitizenship`     | `UI:Tap:PurchaseCitizenship`     | Player taps the "Buy Citizenship" button, before the Yandex payment frame opens. Constant registered in 0019 (payments infra); the button itself is wired in 0018 — the event does not fire until then |
...
Part of the citizenship funnel (`ai-agents/tasks/backlog/0021-analytics-p1-citizenship-funnel/brief.md`). Purchase and XP-earned events are owned by later citizenship tasks and will be added when implemented.
