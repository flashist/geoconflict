...
- Marks dirty-worktree tags explicitly, cleans up the temporary iidfile on all exits, and leaves SSH/SCP plus remote setup as a clear T4e3 stub.

## Outcome

T4e1 is complete. The local workflow preserves the architecture and immutability keepers from the deploy-hardening review reset: an Apple-Silicon host publishes an amd64 artifact, registry credentials stay out of process arguments, and downstream work receives a content-addressed digest rather than a mutable tag. Successful runs stop after push and explicitly report that no VPS was contacted. T4f inserts the secret-byte scan before push; T4e3 later wires transport and remote deployment.
