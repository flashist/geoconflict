...
The original server-visible identity gap is now partially closed by [[tasks/yandex-identity-plumbing]]: T3 carries the Yandex unique ID through match join and stores it on the server-side client. The transported value remains unsigned and untrusted, so paid-citizenship verification still requires the separate Yandex Payments trust boundary. The profile-store identity key is now a keyed hash of the Yandex ID rather than the raw ID; see [[decisions/personal-data-152fz]] and [[tasks/profile-hash-player-ids]]. The updated implementation plan also keeps game/profile failures isolated: profile outages must not stop matches, and game-server crashes must not threaten paid data.
...
- [[systems/player-profile-store]] — current dedicated profile API/Postgres architecture
- [[tasks/profile-backend-db-api]] — T5 implementation of the repository and HTTP API
- [[decisions/personal-data-152fz]] — decision to avoid raw Yandex ID persistence
