...
Added Duos, Trios, and Quads to public server rotation in `src/server/MapPlaylist.ts` while keeping `REGULAR_TEAM_COUNTS` unchanged as `[2, 3, 4]`. Singleplayer and private lobby modal options remain disabled for now because the original low-participant failure is still reachable without public AI fill.
...
Public matchmaking can now generate Duos, Trios, and Quads lobbies alongside regular 2/3/4-team matches and Humans vs Nations. AI Players are compatible because `GameRunner` passes them into `createGame()` as `PlayerType.AiPlayer` entries and `GameImpl.addPlayers()` assigns them through the same team balancing path as human players.

Known limitation: Duos with 1 participant, Trios with 1-2 participants, and Quads with 1-3 participants still throw `Too few teams: 1`. This is acceptable for public matchmaking with AI fill, but these modes should not be exposed in singleplayer or private lobbies until a start guard or equivalent fill path is added.
