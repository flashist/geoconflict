...
- **Player Profile Store T4 was reset and split.** The monolithic backend-infrastructure PR #112 was reverted after an unbounded hardening/review loop. T4 now consists of seven independently shippable slices; T4a's `/health` server skeleton is merged, T4b API-URL config and T4c Dockerfile are ready next, T4d needs VPS/DNS provisioning, and T4e–T4g cover deploy mechanics, image scanning, and argv/concurrency hardening. See [[decisions/profile-deploy-hardening-review-loop]].
...
| Task | Status | Notes |
|---|---|---|
| Player Profile Store — Implementation | in progress | T1 and T3 are done; T2/T7 are cancelled. T4 was reverted and split into T4a–T4g: T4a is merged, T4b/T4c are ready, and T4d needs VPS/DNS before T4e. Then T5 DB/API → T6 crediting → T8 backups |
