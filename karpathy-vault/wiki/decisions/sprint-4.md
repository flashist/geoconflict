...
- **Player Profile Store T4 was reset and split; T4a–T4f are complete, while T4g/T4h/T4i remain.** The monolithic backend-infrastructure PR #112 was reverted after an unbounded hardening/review loop. The bounded restart has shipped the liveness-only profile server, public API-URL runtime config, allowlist-copy amd64 Docker image, dedicated reg.ru VPS provisioning/TLS boundary, local build/push with fail-closed immutable digest resolution, on-box compose plus rollback lifecycle, SSH/SCP deploy wiring, and the image secret byte-scan gate. T4g still hardens argv/concurrency, T4h still propagates `PROFILE_API_URL` through the game-server deploy environment, and T4i is the operator bring-up runbook that turns the merged code into a live `api.geoconflict.ru` 200/TLS milestone. See [[tasks/profile-server-skeleton]], [[tasks/profile-api-url-config]], [[tasks/profile-docker-image]], [[tasks/profile-vps-provisioning]], [[tasks/profile-build-push-digest]], [[tasks/profile-onbox-stack-gate]], [[tasks/profile-deploy-wiring]], [[tasks/profile-image-secret-scan]], [[tasks/profile-server-bring-up-runbook]], and [[decisions/profile-deploy-hardening-review-loop]].
...
| Task | Status | Notes |
|---|---|---|
| Player Profile Store — Implementation | in progress | T1, T3, and T4a–T4f are done; T2/T7 are cancelled. T4g/T4h/T4i remain before T5 DB/API → T6 crediting → T8 backups |
...
- T4e2/T4e3/T4f are done: the profile deploy path now has on-box compose lifecycle, digest rollback, secret-staged SSH/SCP transport, and a blocking built-image layer byte scan. See [[tasks/profile-onbox-stack-gate]], [[tasks/profile-deploy-wiring]], and [[tasks/profile-image-secret-scan]].
- T4i is a runbook, not implementation code. It remains an operator gate before T5 goes live: provision the RU reg.ru host, point DNS, run `npm run deploy:profile`, and verify `https://api.geoconflict.ru/health` returns 200 over valid TLS. See [[tasks/profile-server-bring-up-runbook]].
