...
- **Player Profile Store T4 is complete.** The monolithic backend-infrastructure PR #112 was reverted after an unbounded hardening/review loop, then replaced by bounded T4a–T4i slices: liveness-only profile server, public API-URL runtime config, allowlist-copy amd64 Docker image, dedicated reg.ru VPS provisioning/TLS boundary, local build/push with immutable digest resolution, on-box compose plus rollback lifecycle, SSH/SCP deploy wiring, image secret byte-scan, argv/concurrency hardening, game-server `PROFILE_API_URL` propagation, and live `api.geoconflict.ru` 200/TLS bring-up. See [[tasks/profile-server-skeleton]], [[tasks/profile-api-url-config]], [[tasks/profile-docker-image]], [[tasks/profile-vps-provisioning]], [[tasks/profile-build-push-digest]], [[tasks/profile-onbox-stack-gate]], [[tasks/profile-deploy-wiring]], [[tasks/profile-image-secret-scan]], [[tasks/profile-argv-concurrency-hardening]], [[tasks/profile-game-server-deploy-env]], [[tasks/profile-server-bring-up-runbook]], and [[decisions/profile-deploy-hardening-review-loop]].
- **Player Profile Store T5 is complete.** The profile backend now has a typed-column Postgres migration, repository, `GET /v1/profile`, internal `POST /internal/v1/credit`, and DB-backed `/ready`; game-server match-end crediting remains T6. See [[tasks/profile-backend-db-api]], [[systems/player-profile-store]], and [[decisions/profile-storage-strategy]].
- **Personal-data compliance investigation and ID hashing are complete.** The 152-ФЗ decision is to persist a keyed irreversible hash of the Yandex ID rather than the raw ID; the implementation is done, with the remaining go-live check being on-box verification that no raw Yandex ID exists at rest. Display-name and other PII questions remain open. See [[tasks/personal-data-compliance-investigation]], [[decisions/personal-data-152fz]], and [[tasks/profile-hash-player-ids]].
- **Player Profile Store T2/T7 guest-first slices are cancelled.** The client-only guest localStorage XP store and guest-to-authenticated migration flow were dropped on 2026-06-13 after review-driven hardening expanded the scope beyond the intended small client slice. Profile XP is now authenticated-only until T6 wires match-end crediting; no one earns profile XP before T6. See [[decisions/cancelled-tasks]].
...
| Task | Status | Notes |
|---|---|---|
| Player Profile Store — Implementation | in progress | T1, T3, T4a–T4i, T5, and profile-ID hashing are done; T2/T7 are cancelled. T6 match-end crediting is next, followed by backups before paid citizenship |
...
| Personal-Data Compliance (152-ФЗ) — Investigation | done | Decision: store a keyed irreversible hash of the Yandex ID instead of the raw ID. Remaining open items: display name, email/Telegram PII, minors, written sign-off, and on-box raw-ID-at-rest verification |
...
- The personal-data compliance investigation is separate from VAT/tax and IP/licensing. The raw-Yandex-ID gate is now solved by keyed hashing, but display names and other PII sinks still need explicit treatment before full production clearance
...
- The VAT/tax gate also does not clear Russian personal-data obligations. Sprint 4 resolved the raw-Yandex-ID question by choosing keyed pseudonymization rather than raw ID persistence; display names, email subscription data, minors, written sign-off, and future archive/deletion requirements remain separate compliance concerns.
...
- T4e2/T4e3/T4f/T4g/T4h/T4i are done: the profile deploy path now has on-box compose lifecycle, digest rollback, secret-staged SSH/SCP transport, blocking built-image layer scan, argv-safe/concurrency-safe deploy hardening, game-server `PROFILE_API_URL` propagation, and live `api.geoconflict.ru` TLS health. See [[tasks/profile-onbox-stack-gate]], [[tasks/profile-deploy-wiring]], [[tasks/profile-image-secret-scan]], [[tasks/profile-argv-concurrency-hardening]], [[tasks/profile-game-server-deploy-env]], and [[tasks/profile-server-bring-up-runbook]].
- T5 is done: the profile backend has the first migration, repository, profile read endpoint, internal idempotent credit endpoint, and DB-backed `/ready`; T6 still has to call it from match end. See [[tasks/profile-backend-db-api]] and [[systems/player-profile-store]].
- The guest-first XP path was dropped on 2026-06-13: T2 localStorage-authoritative XP and T7 guest-to-authenticated migration are cancelled, and the `POST /v1/profile/migrate` endpoint was removed from T5 rather than deferred. Guest users still get the locked citizenship card plus login prompt, but profile XP is authenticated-only until T6 server-side crediting ships. A future guest-XP retry should be a thin best-effort cache over the server source of truth, not a localStorage-authoritative store. See [[decisions/cancelled-tasks]].
- The raw-Yandex-ID compliance gate is addressed by keyed hashing; profile go-live still needs on-box raw-ID-at-rest verification and the display-name/other-PII follow-ups recorded in [[decisions/personal-data-152fz]].
...
- [[tasks/profile-argv-concurrency-hardening]] — completed T4g deploy argv/concurrency/target-preflight hardening
- [[tasks/profile-game-server-deploy-env]] — completed T4h game-server env propagation for `PROFILE_API_URL`
- [[tasks/profile-server-bring-up-runbook]] — completed T4i operator runbook and live profile host bring-up
- [[tasks/profile-backend-db-api]] — completed T5 profile DB/API implementation
- [[tasks/personal-data-compliance-investigation]] — completed 152-ФЗ investigation for profile identity handling
- [[decisions/personal-data-152fz]] — accepted profile identity pseudonymization decision
- [[tasks/profile-hash-player-ids]] — completed keyed-hash identity implementation
- [[systems/player-profile-store]] — dedicated profile API/Postgres system
