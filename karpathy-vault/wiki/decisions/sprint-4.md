...
| Investigate & Fix Client Null-ID Errors | backlog | Stabilization follow-up; waits for production source maps and deployed archive-noise cleanup before triage is useful |
...
- The client null-ID/null-object investigation is carried by Sprint 4 as a stabilization follow-up rather than active Sprint 4c work. It should start only after production source maps are live and the archive-noise cleanup is deployed, because the current minified traces and louder archive errors make the cluster uneconomical to triage.
