...
- **Player Profile Store T4 was reset and split; T4a–T4c are complete.** The monolithic backend-infrastructure PR #112 was reverted after an unbounded hardening/review loop. The bounded restart has shipped the liveness-only profile server, public API-URL runtime config, and allowlist-copy amd64 Docker image. T4d still needs VPS/DNS provisioning before T4e deploy mechanics; T4f–T4g then cover image scanning and argv/concurrency hardening. See [[tasks/profile-server-skeleton]], [[tasks/profile-api-url-config]], [[tasks/profile-docker-image]], and [[decisions/profile-deploy-hardening-review-loop]].
...
| Task | Status | Notes |
|---|---|---|
| Player Profile Store — Implementation | in progress | T1, T3, and T4a–T4c are done; T2/T7 are cancelled. T4d needs VPS/DNS before T4e, followed by T4f/T4g. Then T5 DB/API → T6 crediting → T8 backups |
