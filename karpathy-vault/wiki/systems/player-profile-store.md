...
Sources: `ai-agents/knowledge-base/s4-preexisting-infra-impact-2026-06-24.md`, `ai-agents/tasks/done/s4-profile-05-backend-db-api.md`
...
- **Guest path**: the T2/T7 guest-first flow is cancelled. Profile XP is authenticated-only until server-side crediting is complete.
...
- Profile outages must not stop active matches. The profile path is a persistence/monetization dependency, not the gameplay turn loop.
